...
public:
    explicit AAMCCwriter(AAMCCrun* run){
        InitFile((*run));
        InitTrees();
        InitHisto((*run));
        InitRunTree((*run));
    ...
    void operator()(AAMCCEvent* ev, AAMCCrun* run, NucleonVector* nucleons){
        //if(flag){Initialize((*run)); flag = false;}
        FillTrees((*ev));
...
private:
    ...
    void WriteToFile(AAMCCEvent* ev, AAMCCrun* run, NucleonVector* nucleons){
        //std::call_once(flg, Initialize, (*run));
        FillTrees((*ev));
        FillHisto((*ev), (*nucleons));
    };
...
template <class Writer> void wrapWriter(AAMCCEvent* ev, AAMCCrun* run, NucleonVector* nucleons){
    static Writer writer(run);
    writer(ev, run, nucleons);
};