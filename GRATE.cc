...
int main() {
  ...
  histoManager.GetTree()->Branch("Mass_on_A", &AonA);
  histoManager.GetTree()->Branch("Mass_on_B", &AonB);
  histoManager.GetTree()->Branch("Charge_on_A", &ZonA);
  histoManager.GetTree()->Branch("Charge_on_B", &ZonB);
  ...
  AonA = histoManager.GetInitialContidions().GetSourceA();
  ZonA = histoManager.GetInitialContidions().GetSourceZ();
  ZonB = histoManager.GetInitialContidions().GetSourceAb();
  ZonB = histoManager.GetInitialContidions().GetSourceZb();
  ...
  TGlauberMC* mcg = new TGlauberMC(histoManager.GetInitialContidions().GetSysA(),
                                   histoManager.GetInitialContidions().GetSysB(),
                                   signn, omega, seed);
  ...
  if ((histoManager.GetUpB() > 0 && histoManager.GetLowB() >= 0) && (histoManager.GetUpB() > histoManager.GetLowB())
      && histoManager.GetUpB() < rA_plus_rB + 7.5) {
  ...
  if (histoManager.GetStatType() > 2) {
    ...
    ExEnA->SetParametersHybridFit(11.46648905 * MeV,
                                  -1.84830078 * MeV,
                                  -58.53674677 * MeV,
                                  284.66431513 * MeV,
                                  -637.51406293 * MeV,
                                  ...
                                  0.5,
                                  0.2);
    ExEnB->SetParametersHybridFit(11.46648905 * MeV,
                                  -1.84830078 * MeV,
                                  -58.53674677 * MeV,
                                  284.66431513 * MeV,
                                  -637.51406293 * MeV,
  ...
  FermiMom.SetPzPerNucleon(histoManager.GetInitialContidions().GetPzA() / sourceA,
                           histoManager.GetInitialContidions().GetPzB() / sourceAb);
  ...
  for (G4int count = 0; count < histoManager.GetIterations(); count++) {
    ...
    if (!(A == 0 && Ab == 0)) {
      ...
      std::vector<G4FragmentVector> MstClustersVector =
          clusters->GetClusters(&nV, energy_A, energy_B, boostA, boostB); //d = const if energy is negative
      ...
      for (G4int i = 0; i < MstClustersVector.at(0).size(); ++i) {
        ...
        for (auto& product : products) {
          ...
          if (particleEmitted != "gamma" && particleEmitted != "e-" && particleEmitted != "e+") {
            ...
            G4LorentzVector product_p4(product.GetMomentum().x(), product.GetMomentum().y(), product.GetMomentum().z(), eeA);
...
//Side B $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
      ...
      for (G4int i = 0; i < MstClustersVector.at(1).size(); ++i) {
        ...
        for (auto& product : productsB) {
          ...
          if (particleEmittedB != "gamma" && particleEmittedB != "e-" && particleEmittedB != "e+") {
            ...
            G4LorentzVector product_p4b(product.GetMomentum().x(), product.GetMomentum().y(), product.GetMomentum().z(), eeB);
      ...
      if (!G4bool(count % 100)) {
        G4cout << "Program is working," << count << " events calculated    \r" << std::flush;
      }

    }
  }

  G4cout << "----> collided " << histoManager.GetIterations() << " nuclei " << histoManager.GetSysA() << " with "
         << histoManager.GetSysB() << " at N-N x-section " << signn << " mb" << G4endl;
